    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          for await (const event of runDanielAgentStream(message, agentHistory)) {
            if (event.type === "content" && event.content) {
              ...
              const updatedHistory = finalHistory.length > 0
                ? finalHistory
                : [...agentHistory, {
                    role: "user",
              ...
              // Send error
              const errorData = JSON.stringify({ type: "error", error: event.error });
